...
const { getUser, getRoomsByUser, emit2room, getSupportRooms, getRoomInfoById, getLastMessageByRoomid } = require('../utils');
...
const getUsersList = async (req, res) => {
    ...
    data = await Promise.all(data.map(async (item) => {
        ...
        return { ...item._doc, id: item._id, member_id, member_birth, member_company, name, avatar, lastMsg };
    }));
...
const getMessages = async (req, res) => {
    ...
    data = data.map(item => {
        ...
        return { ...item._doc, id: item.id, user }
    });
    res.send(data).status(200);
}
const addNewRoom = async (req, res) => {
    const { room, userid } = req.body;
    if (!room?.id || !userid) return res.send("Invalid user").status(400);
    const { id, avatar, name } = room;
    const data = await RoomCollection.find({
        $and: [
            { $or: [{ members: [id, userid] }, { members: [userid, id] }] },
            { isgroup: false }
        ]
    });
    if (data.length > 0) return res.send({ success: true, roomid: data[0]._id, exits: true }).status(200);
...
const addNewGroup = async (req, res) => {
    const { image, name, members, product_id, isgroup } = req.body;

    var result;
    if(product_id){
        result = await RoomCollection({ members, image, name, isgroup, product_id }).save();
    }else{
        result = await RoomCollection({ members, image, name, isgroup: true }).save();
    }
    const roomid = result._id;

    await RoomSettingsCollection({
        roomid,
        userid: members[0],
        key: 'role',
        value: 'admin',
...
const addMemberGroup = async (req, res) => {
    ...
    var result = await RoomCollection.updateOne({_id: room_id}, {$push: {members: user_id}}, {upsert: false});
...
const uploadFile = (req, res) => {
    ...
    const { body: { path, filename } } = req;

    fileUpload(path, file, filename)
        .then(response => {
...
module.exports = { getUsersList, getSupportRoomList, getMessages, uploadFile, addNewRoom, addNewGroup, addMemberGroup, getRoomInfo };